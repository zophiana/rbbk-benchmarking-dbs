...
/**
 * Utility for benchmarking a set of queries against
 * multiple databases, repeated N times to collect stats
 * including per-query timeout handling.
 */
public class QueryDatabase {
  ...
  /**
   * Benchmarks each SQL in `queries` against each DB in
   * `configs`, repeating each query `runs` times.
   *
  ...
  /**
   * Execute one SQL N times under a per-query timeout.
   * Collects durations (ms), counts timeouts, then logs stats.
   */
  private static void runRepeated(
    ...
    appendLog(logFilePath,
        String.format("INFO: [%s] Avg time: %.2fms",
                      dbName, stats.getAverage()));
    appendLog(logFilePath,
        String.format("INFO: [%s] Median: %.2fms\n",
                      dbName, stats.getMedian()));
  }
  ...
  /**
   * Compute first, last, min, max, avg, median, std-dev
   * over a list of durations (in ms), and count timeouts.
   */
  private static class Stats {
    private final long   first, last, min, max;
    private final double average, median, stdDev;
